...
public class VehicleFieldOfView : AnvilSystem {
	...
	public AnvilVehicle anvilVehicle;
	...
	//	viewMeshFilter.mesh = viewMesh;
		...
		anvilVehicle = GetComponent<AnvilVehicle>();
	...
	void FindVisibleTargets() {
		...
		Collider[] targetsInViewRadius = Physics.OverlapSphere (transform.position, viewRadius, passengerMask);

		//This checks for passengers in the radius
		for (int i = 0; i < targetsInViewRadius.Length; i++) {
			Transform target = targetsInViewRadius [i].transform;
			Vector3 dirToTarget = (target.position - transform.position).normalized;

			//This checks for passengers in the angle
			if (Vector3.Angle (transform.forward, dirToTarget) < viewAngle / 2)
			{
				float dstToTarget = Vector3.Distance (transform.position, target.position);

				//This checks if the passengers is not behind an obstacle
				if (!Physics.Raycast (transform.position, dirToTarget, dstToTarget, obstacleMask))
				{
					visibleTargets.Add (target);
					anvilVehicle.addPassenger(target.gameObject);
